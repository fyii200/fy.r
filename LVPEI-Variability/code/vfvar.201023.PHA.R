...
for (i in 1:length(plot_res$id)) {
  ...
  plot(m.s, var, ylim=c(-3,6))   # no, this doesn't make sense -- because each patient produces only a single point here.
}                                # you need to plot outside the loop...  also: var is a reserved word - check ?var
