...
lv$DOB <- age_calc(lv$DOB, units="years")
...
plfunc <- function(a, b, c, d) {plot (plot_res$mean.s, a, main = paste0(b," Variability vs Mean Sensitivity"), 
...
par(mfrow=c(1,2))
plfunc(plot_res$variab, "", c(0,6), 'blue')
plfunc(plot_res$variabt, "Transformed", c(0,45), 'red')
dev.off()
